...
import { FaEdit, FaTrash, FaWallet, FaInbox } from "react-icons/fa";
...
export default function ListAkunSection({ listAkun, onEdit, onHapus }: Props) {
    return (
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {listAkun.length === 0 ? (
                <div className="col-span-full flex flex-col items-center justify-center bg-white dark:bg-gray-800 p-6 rounded-lg shadow-sm border border-dashed border-gray-300 dark:border-gray-700 text-center">
                    <FaInbox className="w-10 h-10 text-gray-400 dark:text-gray-500 mb-2" />
                    <p className="text-sm text-gray-500 dark:text-gray-400">Belum ada akun yang ditambahkan</p>
                </div>
            ) : (
