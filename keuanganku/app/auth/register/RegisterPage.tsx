...
import { handler_AuthDaftar } from '@/actions/v2/handlers/auth';
    ...
    handler_AuthDaftar(
      {
        ...
        toaster: toast,
        whenSuccess: () => {
          router.push((WEB_ROUTE.AUTH.LOGIN))
        }
      },
      {nama, email, password}
    )
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 transition-colors duration-300 px-4 sm:px-6">
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 text-black dark:text-white rounded-xl shadow-md p-6 sm:p-8">
        ...
        <p className="text-sm text-center mt-6">
          Sudah punya akun?{' '}
          <a href={WEB_ROUTE.AUTH.LOGIN} className="text-indigo-500 hover:underline font-medium">
            Masuk di sini
          </a>
        </p>
      </div>
